...
- **`lin-ir`** — flat 3-address IR (`LinIR`) sitting between `TypedExpr` and LLVM, and the **sole** lowering path. Contains: IR data types (`ir.rs`), the `TypedModule → LinModule` lowering pass (`lower.rs`, incl. `lower_module` for the main module and `lower_import_module` for imports), backwards-dataflow liveness analysis (`liveness.rs`), and the Perceus-inspired RC elision pass (`rc_elide.rs`). Note: `lin-ir` also deliberately hosts the **monomorphizer** (`monomorphize.rs`, `TypedModule → TypedModule` generic specialization) — it operates on `lin-check` typed IR but lives here because it runs immediately before lowering, as part of the same pipeline stage.
...
Stdlib lives in `stdlib/*.lin` and is loaded via `include_str!` in `lin-compile`. Current stdlib modules: `std/io`, `std/string`, `std/number`, `std/iter`, `std/array`, `std/object`, `std/async`, `std/fs`, `std/stream`, `std/compress`, `std/archive`, `std/http`, `std/net`, `std/process`, `std/tty`, `std/signal`, `std/template`, `std/test`, `std/time`.
...
- **Prefer generics and named/intersection (`&`) record types over `AnyVal` in `.lin` code.** `AnyVal` is a dynamic escape hatch for genuinely unknowable shapes (untyped wire data, recursive ASTs) — it disables type checking and is a perf cliff (field access on `AnyVal` is an optimisation barrier; typed records inline to a constant slot load). Default to monomorphised generics `<T>` for shape-agnostic code and named records (composed with `&`, ADR-061) for known shapes.
...
- **Iterator semantics** — `lin_iter` / `lin_for` intrinsics in `lin-runtime`; `lin_iter` constructs an opaque iterator handle. 
