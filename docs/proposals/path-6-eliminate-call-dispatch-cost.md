...
## 6c IMPLEMENTATION FINDINGS (2026-06-09, branch `worktree-agent-abd709e0a200824aa`, commit `d66c5f9d`, NOT merged)

**Verdict: 6c is a no-win — ~2-3% interp shaving, 0% on fused workloads, cannot move RAPTOR.** Confirmed via the orthogonal Strategy-B route, matching the LTO and bitcode-runtime attempts.

**Strategy chosen: B (emit-at-call-site), not A (bitcode+alwaysinline).** A was already proven by the `spike/bitcode-runtime` Tier-1 spike (`bf72f308`) to fire the *visibility* mechanism (helper calls 91→0) but **not cancel box/unbox pairs**, at high integration cost (hash-mangled internal statics, "genuinely fragile"). B is the sound, zero-build-change version: emit the trivial scalar unbox helpers (`lin_unbox_int32/int64/uint64/float64/bool`) as an inline `load` of the `TaggedVal` payload (offset 8) at the codegen call site; null-branching helpers (`lin_unbox_ptr`/`lin_get_tag`) stay as calls. Gated `LIN_INLINE_UNBOX` (default ON). Files: `codegen/boxing.rs`, `codegen/data.rs`.

**IR proof the mechanism fires but pairs do NOT cancel:** dynamic-Json reduce probe scalar-unbox calls 57→5 (156 inline loads); RAPTOR 808 inline loads. But the user reduce closure `acc + i` is post-O2 **byte-identical** in both modes (`box_int64 → lin_tagged_arith → free_box`) — because `lin_tagged_arith` (the consumer) is an opaque optimisation barrier, so LLVM cannot prove the box doesn't escape and cannot cancel it. RAPTOR's `lin_object_get`(253)/`lin_map_get`(205)/`lin_tagged_arith`(28) untouched. And the already-fused 6a/in-place scalar loops are pure `load→mul→icmp→add` with **zero box/unbox**, so 6c has nothing to inline there.

**Measured (verified low load):** interp +3.4% median (call-boundary removal, real but tiny); dynamic-Json reduce +1.3% (in noise — the no-cancellation outcome directly); pipeline 0% exactly (fully fused, nothing to inline); RAPTOR digest byte-identical, no trustworthy wall-clock A/B obtainable (load 26-55 throughout) but IR evidence decisive — representation bottleneck untouched. ASan clean both modes, leak constant 1322 B at N=1k AND N=100k, identical to inline-OFF control. 687 integration + 72 `.test.lin` pass.

**Mechanism reason (the general lesson):** inlining the unbox *producer-side read* removes a call/return boundary (hence ~2-3% interp shaving) but **cannot trigger box/unbox cancellation while the consuming op stays opaque** (`lin_tagged_arith`, `lin_object_get`, the indirect closure call). Where box→unbox *would* be adjacent, 6a already eliminated them at the IR level. **6c only pays AFTER the consuming boundary falls** — i.e. it's a finishing pass for Tier-3 devirtualization/monomorphization of the consuming helper (interp) and end-to-end packed records / Path-9 (RAPTOR), neither of which is done. The change is sound and can sit behind the flag as ready infrastructure, but is **not a standalone merge.**

**This closes path-6's chartable mechanisms: 6a (fusion) + 6b (dispatch) + in-place base MERGED and real; 6c attempted and confirmed no-win standalone.** The frontier moves off path-6 to Tier-3 (consuming-helper devirtualization) and Path-9 (end-to-end packing) — the two levers that remove the *opaque consumer*, which is the actual barrier every box/unbox-cancellation attempt has hit.
