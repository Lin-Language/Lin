...
> ### ⚠️ CORRECTION 2026-06-09 — the Phase-0 call-site-CLASS profile partly FALSIFIES the "interp records are sealed → reads cheap" cell above
> The cost-map's interp column assumed interp's records are sealed and so its `OBJECT_GET` is ≈0 (cost = calls only). The per-call-site-CLASS profiler (`investigate/raptor-callsite-class`, `5353d8eb` — a `lin_count_object_get_class` emitted at every codegen index/field-get site, classifying by *static* receiver type) **measured otherwise**: interp's `OBJECT_GET` is **4.93 M, 100% RECORD-class, all linear-scan** — its `Token` (boxed `Token[]`), `Cursor.node` heap field, and `Ast` sum-node project path are **NOT actually sealed at codegen**. So interp is **not purely call-bound** — it carries a real boxed-heap-field-record read cost *on top of* its call/RC churn (RC_RETAIN 23.8 M, ALLOC_TAGGED 5.4 M). **Net: the two programs share *one* root frontier — the boxed heap-field record — more than this map's clean "interp=calls / RAPTOR=repr" split implied.** Path 8's call-axis levers still address interp's churn half, but [Path 9](path-9-end-to-end-packed-records.md)'s heap-field packing addresses interp's record half too — it is the dominant *shared* lever, not RAPTOR-only. The RAPTOR column is confirmed and *sharpened*: of `lin_object_get`, **20% RECORD + 80% OPAQUE = 100% record-shaped, 0% genuine-dictionary** — the 20.3% `{String:T}` dict bucket is already O(1)-banked, so **no cheap dict-typing win remains**; every residual `object_get` needs packing. See [[project_phase0_callsite_class_profile]].
...
### ❌ Tier 3 (devirtualize statically-known calls) — DEAD END, measured (spike branch `perf/path8-tier3-devirt`, NO commits — verdict only)
The remaining unbuilt Tier-3 lever — turn a statically-known indirect closure call into a **direct** call so LLVM can inline it — was spiked measure-first and is **not worth pursuing**. The premise is already satisfied:
- **Named / top-level / imported function calls are ALREADY emitted as direct LLVM calls.** The IR lowering (`crates/lin-ir/src/lower.rs`) resolves a top-level `val` callee via `global_fn_slots` → `CallTarget::Direct(fid)` (~line 4370) and an imported fn via `import_fn_slots` → `CallTarget::Named(sym)` (~line 4268), both lowering to `call @fn`. Only the fallthrough (~line 4478) is a boxed-closure `indirect_call`.
- **interp's entire recursive hot path is 100% direct** (`evalNode`→`evalNode`, `parseExpr`/`parseTerm`/`parseFactor`, `tokenize`). The only indirect call it *executes* is the single outer `exprs.for(closure)` dispatch (~80 k invocations) vs millions of direct eval/parse calls underneath — negligible.
- **The 55 indirect calls in `interp.ll` all live inside stdlib combinator bodies** where the callback `f` is a genuine opaque function *parameter* — not statically knowable at the combinator's lowering site. (The call-*site* bare-fn case is already handled by the merged eta-expansion, `eda65f31`/`07936afe`/`fe270f6a`.)
- **The two genuinely-devirtualizable patterns have a near-zero population:** a local `val f = (x)=>…; f(x)` (3 occurrences corpus-wide, none hot) and a user higher-order fn with a fn-typed param called in its body (3 occurrences — `outcome.andThen`/`mapOk`, `web-server.makeRouter`, `gtfsLoader.forEachRow` — none in tight loops; fixing them needs callee inlining, not call-site devirt).

**Verdict (bounds Path 8):** combined with the Phase-0 record-read dominance, there is **no broad-applicability call-cost win left to recover.** Tier 3 devirtualization is closed-negative; Tier 1 (bitcode) was already closed-negative (`spike/bitcode-runtime`); path-6's 6c (leaf-helper inlining) was closed-negative. Path 8's *live* remaining value is Step 8.1 (record-combinator-chain fusion, ~2.3×, secondary) as shared groundwork for Path 9. **Stop proposing call-axis levers for the headline benchmarks — the cost is the boxed heap-field RECORD representation (Path 9), not the calls.**

> **Corroboration (independent rediscovery): the bare-fn devirtualization wedge is genuine.** A separate fan-out re-found the `xs.map(square)` ~1.5× gap (bare-fn callback = closure-shell alloc + indirect dispatch vs the lambda's direct call) from scratch and built the same fix (`bare_fn_call_target` → `CallTarget::Direct`/`Named` at the combinator site; branch `perf/devirtualize-barefn-combinator`, ~2.4×, IR-proven, ASan-clean) — before confirming it was **already merged** as the eta-expansion (`eda65f31`/`07936afe`/`fe270f6a`, extended to `for`/`while`). Two independent agents reaching the same fix is confidence it's correct; the merged version is more complete, so the spike branch is superseded. This is the *only* call-axis win that paid — and it's the call-*site* case, already shipped. It does **not** generalise to the opaque-callback-*parameter* indirect calls inside combinator bodies (those need callee inlining, near-zero hot population). The verdict stands.
