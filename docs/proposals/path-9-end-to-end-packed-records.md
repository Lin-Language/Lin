...
## 🏁 AGENT-1 FINDINGS 2026-06-10 — first to BAIL on heap-field packing; verdict CLOSED-NEGATIVE for the packing thesis, but THREE standalone wins salvaged

This is one of three parallel Path-9 explorations. I (agent-1) carried the heap-field-packing chain to a working end-to-end RAPTOR and am the first to conclude **the packing thesis is closed-negative** — while extracting three genuinely useful, master-independent wins along the way. All branches below are UNMERGED. Everything claimed here I verified MYSELF (built + ran + measured at low load), not from agent self-reports — distinguished explicitly.

### The verdict on the packing thesis: CLOSED-NEGATIVE (a built, measured dead end — not a guess)
End-to-end heap-field record packing makes **leaf/scan reads const-offset** (proven at every step: `scanBack` object_get 8→0, `scanRouteAt` 6→2, IR-verified) — but it is a **net regression end-to-end**, and the implementation chain is **fix-for-a-fix all the way down**:
- The typed-packed RAPTOR now RUNS (after fixing a crash, below) but **PREP regresses badly** — it does not complete in 300s where master's PREP is ~29s (a REGRESSION, confirmed by Linus; I verified PREP hangs after LOAD).
- **Each packing fix only repairs a bug a *prior* packing fix introduced.** Decisive proof: the "TCO borrowed-param UAF" I fixed (`fadd5ebf`, ASan fails-before/clean-after, real) **cannot occur on master** — master's `tco_param_needs_release` carves out sealed records entirely (`&& sealed_fields(ty).is_none()`), so the repro prints `done`/exit 0 on master. The UAF only exists *because* an unmerged packing fix (heap-bearing-sealed TCO release) introduced it. The same holds for the nested-record gate, the worker-boundary REPR guard, the map-value seam: each is a patch for damage the packing introduces.
- **The mechanism of the dead end:** a functional language threads records through MANY generic boundaries (maps, combinators, unions, TCO params). Each boundary is a separate materialize-or-leak site. Packing makes the *read* cheap but the per-boundary *materialization* it forces costs more than the read saves — and patching one boundary reveals the next. The seam sequence peeled: worker-boundary → nested-record gate → TCO param leak → TCO borrowed-param UAF → map-value materialize → **generic combinators over packed record arrays materialize+leak the whole array per call** (the PREP blocker; `createRaptor` runs `sorted.for`/`stopTimes.for`/`existing.some`/`sort` over 240k packed trips). This last seam is where I bailed: fixing it is a broad, risky change to combinator monomorphization's element-boxing release path, and the pattern says another seam likely waits behind it.
- **This matches the GC (closed-negative), value-types (breaking), Tier-1 bitcode (<2%), Tier-3 devirt (dead-end) results** — the read-cost lever everyone kept betting on is not where the win is. The cost is representation-boundary materialization, not the reads.

**Recommendation for assessment:** do NOT merge the packing chain. The `perf/path9-reconciled-mega` (`5dcfda53`) capability branch + `perf/path9e-mapvalue-keeppacked-final` (`62d624fa`) end-to-end branch are net-negative-over-master as perf and should be retired unless another agent's line measures an actual end-to-end win that this line did not reach.

### ✅ THREE standalone wins salvaged (all verified by me on CURRENT master, all independent of packing)
1. **Step 8.1 — record-combinator chain fusion. Branch `perf/path8-step1-record-fusion` (`cf1c07e6`; core `cda7e95b`+`4f37b2aa`).** Widens the merged Tier-2 fusion to sealed-RECORD element arrays + array-producing terminals: a `recs.filter(…).map(…).reduce(…)` over a record array becomes a single const-offset pass, no per-stage intermediate array. **MEASURED ~2.3× (master 6.7s → 2.95s, 2M-record chain, identical output, median of 5 at load <2).** `lin-ir/src/lower.rs`-only, merges clean onto master, 705 integration + 72/72 lin + fmt green (I ran them). Includes two real pre-existing leak/UAF fixes in the multi-stage fuser (`4f37b2aa`). **The strongest keeper.**
2. **9C seal-propagation. Branch `perf/path9c-seal-propagation` (`e745c367`; commits `253ea5f6`+`a26a2cc9`).** Checker-only (`expr.rs`). Fixes **LIVE silent data corruption on current master** — a nested all-scalar sealed-record-array read `outer["items"][0]["a"]` prints garbage `7 0` on master, `33 44` with this fix (I re-confirmed `7 0` on current master). Gate stays scalar+Bool; producer seals to match consumer. 696 integration + 72/72 + RAPTOR 9/9 green. **A correctness fix worth landing regardless of any perf direction.**
3. **Map-value direct-index — a reusable lever, MEASURED standalone.** The Path-9 seam fix (`1217cd45`) replaced `get<T,D>(m,k,default)` over a typed `{String:T[]}` map with a direct typed index + null-narrow. Isolated on PLAIN master (no packing): `get<T,D>` 2.19s → direct `m[k]`+narrow **1.41s (~1.55×)**, identical output, IR boxes 220→97 / object_get 6→1. Root cause = the generic `get<T,D>` wrapper (object.lin:133) boxes the `{String:T}` value + the `T|D` union coercion rebuilds it per call — pure monomorphization-wrapper overhead, present on master regardless of packing. CAVEAT: measured the **array/heap-value** case; a scalar-valued map get likely shows less. Two ways to bank it: (a) userland — rewrite hot `get(map,k,d)` to direct-index where default only covers missing keys; (b) **codegen — make `get<T,D>` itself lower to the keep-packed direct read** so all callers benefit (higher leverage, more work). Not yet on a clean branch; the technique lives in `1217cd45`'s RAPTOR `.lin` edits.

### Branch ledger (this line, all UNMERGED, off various bases — see each)
| Branch | Tip | Carries | Verdict |
|---|---|---|---|
| `perf/path8-step1-record-fusion` | `cf1c07e6` | record-combinator chain fusion (~2.3×) | ✅ MERGE — standalone, verified, clean |
| `perf/path9c-seal-propagation` | `e745c367` | nested sealed-record seal-propagation | ✅ MERGE — fixes live 7/0 corruption |
| `fix/raptor-sealed-release-misalign` | `fadd5ebf` | TCO borrowed-param UAF fix | ⚠️ only needed IF packing's TCO-sealed-release lands; not a master bug |
| `perf/path9-reconciled-mega` | `5dcfda53` | consolidated String-packing capability | ❌ packing thesis — net negative; retire |
| `perf/path9e-mapvalue-keeppacked-final` | `62d624fa` | end-to-end RAPTOR + map-value seam fix + the direct-index technique | ❌ packing; but mine the map-value technique (~1.55×) out of it |
...
## 🏁 AGENT-3 FINDINGS 2026-06-10 — the orchestrated Phase-0→9-E→endgame line; CLOSED-NEGATIVE CONFIRMED (third independent agreement) + one distinct mechanism the others didn't isolate
This is the managed/orchestrated line (Phase-0 profiler → 9-A/9-A-cont/9-B stack → 9-E map-value → the combined `combine/path9-raptor-payoff-final` branch → the endgame). It reached the **same CLOSED-NEGATIVE verdict by a different branch construction**, which is the third independent corroboration. It also surfaced findings the other two lines did not.

### Corroboration of the verdict (same destination, independent route)
- Built the consolidated capability branch `combine/path9-raptor-payoff-final` (full 9-A→face-2→String→nested+Map gate→9-E map-value keep-packed→Fix1 nested-record gate→Fix2 TCO-param-slot leak), rebased clean onto current master incl. master's `63a25e0d` write-sink fix + the ~23% RAPTOR perf rewrite (`94295e88`). Verified by me: **cargo test 701/0, lin test 72/72, all-Json digest byte-identical.**
- The typed-RAPTOR retype on this line **regresses / does not complete cleanly** — same destination as agent-1 (bailed) and agent-2 (measured ~1.8× slower, completing). Three lines, three branch constructions, one verdict.

### Distinct contributions this line added (not found by the other two)
1. **Fix2 — TCO param-slot drop leak for heap-bearing sealed records is REAL, FIXED, and PROVEN CONSTANT** (`codegen/types.rs`, commit `441287b7`, on the combined branch). The carve-out `tco_param_needs_release … && sealed_fields(ty).is_none()` excluded every sealed record from TCO param-slot release → a packed record threaded through a TCO loop leaked ~6568 B/iter scaling. After fix: 367 B/iter scaling → **129 B CONSTANT** (N=50/1000/10000), correct, no UAF. **A standalone correctness keeper independent of the packing verdict.**
2. **The get-seam union keep-packed codegen fix is sound and a strict improvement** (`fdcc7120`, `codegen/match.rs`+`boxing.rs`): a packed-sealed-array → `T[]|D` union widen (the `get<T,D>` result) now wraps keep-packed (`compile_ir_box_keep_packed_owned`, RC-balanced +1 matching the materialize path) instead of `sealed_array_to_tagged`. ASan-clean, **reduces** leaks (222KB vs 408KB on the seam test), and **fixes a real `get(map,k)→push` silent write-loss**. Keeper regardless of the packing verdict. (This is the codegen form of agent-1's userland "map-value direct-index ~1.55×" lever — same root cost, different layer.)
3. **A distinct crash seam the other lines didn't isolate — the `scanRouteAt` TCO pass-through over-release.** Deep IR+ASan tracing on this line's typed RAPTOR found a *second* sealed.rs:133 crash (heap-UAF of the 48-byte `ScanResults`) rooted NOT in the union repr but in `release_owned_for_tail_call` (`lin-ir/src/lower.rs:1242–1252`): it applies the per-registration sealed release rule to a **borrowed pass-through** TCO param (`results` threaded unchanged through `scanRouteAt`'s ~8 tail branches), over-releasing it to rc 0 mid-loop while a `.for` closure still captures it. The correct fix needs per-branch retain↔registration counting (or suppressing read-retains on sealed pass-through params, like the Json/union path). **Confirms agent-1's "fix-for-a-fix all the way down" prediction concretely** — this is a *third* distinct materialization/RC seam beyond the union seam and the map seam.

### Process note (a real measurement hazard this line hit)
The bash tool's cwd was non-deterministic between calls; several `cargo build benchmarks/…` / `Read` calls silently hit the **main `master` checkout** (which carries a *different, working* RAPTOR) instead of the worktree, producing a transient false "crash is gone." **Always build/measure RAPTOR with absolute paths into the intended worktree** — a cross-checkout artifact mimics both false wins and false fixes. Also caught: an earlier line's typed `.lin` **never type-checked** (`val d: {String:Boolean} = dRec["dates"]` reads a typed map off a `Json` result, spec §5.1.1) and had **reverted master's ~23% perf rewrite** — so any "9-D measured" claim predating `1516a968`/`e74ca0ff` on a non-type-checking source is suspect. Both fixed on this line. See [[project_path9_raptor_crash_real_seam]] and [[project_path9_combined_branch_verdict]].

### Net (agent-3)
**Third independent agreement: end-to-end heap-field packing for RAPTOR is CLOSED-NEGATIVE.** The cost is representation-boundary materialization at *multiple* seams (map-value, `Trip|Null` union, TCO pass-through) — fixing each reveals the next, exactly as predicted; PREP's 3.5× regression isn't even a union/read seam (it's loader/construction). **Standalone keepers this line adds to the salvage list:** the **Fix2 TCO-param-slot leak fix** (constant-leak correctness, mergeable) and the **get-seam union keep-packed codegen fix** (`fdcc7120` — RC-balanced, leak-reducing, fixes a write-loss bug). Both are correctness wins worth landing independent of the (retired) packing perf thesis. All branches UNMERGED.
