...
- **(c) `Error` is a structural object alias, with a width-subtyping wrinkle.** Because `Error` is `{type, message}` (open), it is *not* a nominal tag: `value is Error` compiles to a runtime **tag check only** (`compile_ir_is_type` checks `TAG_OBJECT`, not field shape), so `is Error` matches *any* object — including a successfully decoded object type. Therefore the **robust idiom for detecting a decode failure is the discriminant `result["type"] == "error"`** (or `has { "type": "error" }`), not `is Error`. Symmetrically, a successfully decoded type that happens to look like `{type, message}` would satisfy `is Error`. This is low-risk in practice (you decode known types) but is the price of modelling `Error` structurally instead of adding a dedicated `Type` variant; it is documented so users reach for the discriminant.
