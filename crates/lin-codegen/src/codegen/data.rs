...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn compile_ir_field_get(&mut self, obj: BasicValueEnum<'ctx>, field: &str, obj_ty: &Type, result_ty: &Type, obj_repr: &lin_ir::repr::Repr) -> BasicValueEnum<'ctx> {
        ...
        if obj.is_pointer_value() {
            ...
            // TAG_MAP, TAG_OBJECT, or TAG_RECORD (e.g. a sealed record laundered through AnyVal,
            // then read by a `match has { "type": ... }` discriminant FieldGet). A blind
            // `lin_map_get` on the unboxed pointer treats a sealed-struct / LinObject as a LinMap
            // and crashes (`find_slot_string` on garbage). Delegate to the Index path, which
            // tag-dispatches map/object/record exactly like `obj[field]` does. The packed-struct /
            // sumnode / nullable-record reprs were already handled by the early returns above, so
            // `obj_repr` here is the boxed dynamic case those Index branches correctly skip.
            if Self::is_union_type(obj_ty) {
                let key_str = self.compile_string_lit(field);
                return self.compile_ir_index(obj, key_str, obj_ty, &Type::Str, result_ty, obj_repr);
            }
            // Phase 2: concrete (non-union) open-object container is a LinMap*; use map_get.
            let container = obj;
            let key_str = self.compile_string_lit(field).into_pointer_value();
