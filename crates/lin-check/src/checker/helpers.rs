...
/// Non-transferable: Function, Iterator, Stream, cursor-sharing Opaque handles, Never.
...
/// `Opaque("TarEntry")` shares a live cursor into the parent stream — it is neither
/// deep-copyable nor movable across a thread boundary.
pub(crate) fn is_definitely_non_transferable(ty: &Type) -> bool {
    match ty {
        Type::Function { .. } | Type::Iterator(_) | Type::Stream(_) | Type::Never => true,
        Type::Opaque(n) if n == "TarEntry" => true,
...
/// via CAP_MOVE. Only opaque cursor-sharing handles (`Opaque("TarEntry")`, `Iterator`) are
/// rejected at the capture site.
...
/// `TypedExpr::Function` whose type is non-transferable (i.e. `Opaque("TarEntry")` or
/// `Iterator`), or `None` if there are none. Used by the async-thunk capture gate.
///
