...
///
/// Null-safe, cached-box-safe (immutable static scalar boxes are never freed).
#[no_mangle]
...
mod cache_tests {
    ...
    fn release_if_distinct_reclaims_inner_when_distinct() {
        unsafe {
            ...
            // Some OTHER distinct pointer standing in for the callback-return box.
    ...
    fn release_if_distinct_scalar_box_degrades_to_shell_free() {
        unsafe {
            ...
            // Use out-of-range ints that heap-allocate.
