...
```lin
u32ToLe(0x11223344)   // [0x44, 0x33, 0x22, 0x11]
```
...
`f32ToBits` and `f64ToBits` expose the IEEE-754 bit pattern of a float (with the `*FromBits`
...
Reinterpret the bit pattern of a Float32 as a UInt32 (IEEE-754 bits).
- **`f`** — the float.
...
Reinterpret the bit pattern of a Float64 as a UInt64 (IEEE-754 bits).
- **`f`** — the double.
...
```lin
val bytes: UInt8[] = f64ToBe(3.14159)   // 8 bytes
```
